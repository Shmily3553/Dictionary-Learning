...
im_name = 'images/peppers.png';   % input image
% std_noises = [5,10,15,20,25,30];     % standard deviation of Gaussian noise
std_noises = [10];
im_gt = double(imread(im_name));    % ground truth image
pnsrs = [];
for sn = 1:size(std_noises,2)
    std_noise = std_noises(1,sn);
    ...
    %% set parameters
    % % algos = ["mod","ksvd"];
    algos = ["overcomplete","mod","ksvd"]; % the algorithms for running
    ...
    n_iter = 25;    % maxiaum number of iterations in dictionary learning
    ...
    % Setting parameters for ksvd
    opts_ksvd.K = n_atom;
    opts_ksvd.nbr_max_atoms = topK;
    opts_ksvd.niter_learning = n_iter;
    opts_ksvd.learning_method = 'ksvd';
    
    % Setting parameters for mod
    opts_mod.K = n_atom;
    opts_mod.nbr_max_atoms = topK;
    opts_mod.niter_learning = n_iter;
    opts_mod.learning_method = 'mod';
    ...
    for k = 1 : size(algos,2)
        ...
        for t = 1:ts
            ...
            D=selectDlFunc(Y, lambda, D0, C0, opts_dl, opts_ksvd, opts_mod, UA, std_noise, DD, algos(1,k), topK, mu, lambda2, n_atom);
            ...
            sprintf("%s with psnr: %f", algos(1,k), pnsr);
            pnsr_(t) = pnsr;
            save('./results/psnr_', 'pnsr_');
        end
        disp('=============================PNSR===================================');
        pnsr_mean = mean(pnsr_);
        disp(pnsr_mean);
        pnsrs = [pnsrs; pnsr_mean];
        
        % % save figure
        % fig=figure;
        % imshow(uint8(im_r));
        % folder = './results/lena';
        % filename = sprintf('noise%d.pdf', std_noise);
